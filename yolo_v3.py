...
class Yolo(nn.Module):
    ...
    def IOU(self, b1, b2):
        ...
        CM = (min(b1[2], b2[2]) - max(b1[0], b2[0])) * (min(b1[3], b2[3]) - max(b1[1], b2[1]))
        return (CM) / (A + B - CM + 0.0001)
    ...
    def forward(self, batch_x, batch_box):
        ...
        for x, box in zip(batch_x, batch_box):
            ...
            for abox in box:
                index = self.IOU_index(abox[3], abox[4])
                if index >= self.s_index and index <= self.e_index:
                    ...
                    iou = self.IOU(
                        ...
                        [
                            alpha_x * div - abox[3] / 2,
                            alpha_y * div - abox[4] / 2,
                            alpha_x * div + abox[3] / 2,
                            alpha_y * div + abox[4] / 2,
                        ],
                    )
